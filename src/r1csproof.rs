...
use crate::prova4::R1CSVerificationCircuit2;
...
use digest::generic_array::typenum::Abs;
...
pub struct R1CSProof<E: Pairing, P: Pairing> {
  // The PST commitment to the multilinear extension of the witness.
  pub comm: Commitment<E>,
  sc_proof_phase1: crate::sumcheck2::SumcheckInstanceProof<E>,
  claims_phase2: (
    E::ScalarField,
    E::ScalarField,
    E::ScalarField,
  ...
  sc_proof_phase2: crate::sumcheck2::SumcheckInstanceProof<E>,
...
pub struct R1CSVerifierProof<E: Pairing, P: Pairing> {
  comm: Commitment<E>,
  circuit_proof: ark_groth16::Proof<P>,
...
#[derive(Clone)]
pub struct CircuitGens<E: Pairing, P: Pairing> {
  pk: ProvingKey<P>,
  vk: VerifyingKey<P>,
  _e: PhantomData<E>,
}

impl<E, P> CircuitGens<E, P>
where
  E: Pairing,
  P: Pairing,
  P: Pairing<ScalarField = E::BaseField>,
{
  ...
  pub fn setup(
    ...
    let circuit = R1CSVerificationCircuit2 {
      num_vars: num_vars,
      num_cons: num_cons,
      input: (0..num_inputs)
      ...
      sc_phase1: crate::prova4::SumCheckVerificationCircuit::<E> {
        polys: uni_polys_round1,
      },
      sc_phase2: crate::prova4::SumCheckVerificationCircuit::<E> {
...
#[derive(Clone)]
pub struct R1CSGens<E: Pairing, P: Pairing> {
  gens_pc: PolyCommitmentGens<E>,
  gens_gc: CircuitGens<E, P>,
}

impl<E: Pairing, P: Pairing> R1CSGens<E, P>
where
  P: Pairing<ScalarField = E::BaseField>,
{
  ...
  P: Pairing,
{
  fn prove_phase_one(
    ...
    crate::sumcheck2::SumcheckInstanceProof<E>,
    Vec<E::ScalarField>,
    Vec<E::ScalarField>,
  ) {
    let comb_func =
    ...
    let (sc_proof_phase_one, r, claims) =
      crate::sumcheck2::SumcheckInstanceProof::<E>::prove_cubic_with_additive_term(
    ...
    E::ScalarField: Absorb,
  {
    ...
    let (sc_proof_phase_two, r, claims) = crate::sumcheck2::SumcheckInstanceProof::prove_quad(
    ...
    E::ScalarField: Absorb,
  {
    ...
    let (sc_proof_phase1, rx, _claims_phase1) = R1CSProof::<E, P>::prove_phase_one(
      num_rounds_x,
      &mut poly_tau,
      &mut poly_Az,
      &mut poly_Bz,
    ...
    let (sc_proof_phase2, ry, _claims_phase2) = R1CSProof::<E, P>::prove_phase_two(
      num_rounds_y,
      &claim_phase2,
  ...
  pub fn prove_verifier(
    ...
    gens: &R1CSGens<E, P>,
    poseidon: PoseidonConfig<E::BaseField>,
  ) -> Result<R1CSVerifierProof<E, P>, ProofVerifyError>
  where
    P: Pairing<ScalarField = E::BaseField>,
    E::ScalarField: Absorb,
  {
    ...
    let config = crate::prova4::VerifierConfig {
      num_vars,
      num_cons,
      input: input.to_vec(),
      evals: *evals,
    ...
    let circuit = R1CSVerificationCircuit2::new(&config);

    let circuit_prover_timer = Timer::new("provecircuit");
    let proof = Groth16::<P>::prove(&gens.gens_gc.pk, circuit, &mut rand::thread_rng()).unwrap();
...
impl<E: Pairing, P: Pairing> R1CSVerifierProof<E, P>
where
  <E as Pairing>::ScalarField: Absorb,
{
  ...
  pub fn verify(
    ...
    // ]);
    transcript.new_from_state2(&self.transcript_sat_state);
    par! {
      // verifies the Groth16 proof for the spartan verifier
      //let is_verified = Groth16::<P>::verify(&gens.gens_gc.vk, &pubs, &self.circuit_proof).unwrap(),
...
mod tests {
  ...
  fn check_r1cs_proof_bls12_377() {
    let params = crate::parameters::get_bw6_fr_params();
    check_r1cs_proof::<ark_bls12_377::Bls12_377, ark_bw6_761::BW6_761>(params);
  }
  ...
  //   }
  fn check_r1cs_proof<E, P>(params: PoseidonConfig<P::ScalarField>)
  where
    E: Pairing,
    ...
    P: Pairing<ScalarField = E::BaseField>,
  {
    ...
    let num_inputs = 3;
    let (inst, vars, input) =
      R1CSInstance::<E::ScalarField>::produce_synthetic_r1cs(num_cons, num_vars, num_inputs);

    let gens = R1CSGens::<E, P>::setup(b"test-m", num_cons, num_vars, num_inputs, params.clone());
...
// use ark_crypto_primitives::sponge::Absorb;
// use ark_ec::pairing::Pairing;
// use crate::circuit_verifier::TestudoCommVerifier;
// use ark_poly_commit::multilinear_pc::data_structures::{Commitment, Proof};
// use itertools::Itertools;
...
//   // The PST commitment to the multilinear extension of the witness.
//   pub comm: Commitment<E>,
//   sc_proof_phase1: SumcheckInstanceProof<E::BaseField>,
//   claims_phase2: (
//     E::BaseField,
//     E::BaseField,
//     E::BaseField,
//     E::BaseField,
//   ),
//   sc_proof_phase2: SumcheckInstanceProof<E::BaseField>,
//   pub eval_vars_at_ry: E::BaseField,
//   pub proof_eval_vars_at_ry: Proof<E>,
//   rx: Vec<E::BaseField>,
//   ry: Vec<E::BaseField>,
//   // The transcript state after the satisfiability proof was computed.
//   pub transcript_sat_state: E::BaseField,
//   pub initial_state: E::BaseField,
//   pub t: E::TargetField,
//   pub mipp_proof: MippProof<E>,
// }
...
//   E: Pairing,
//   P: Pairing,
//   R1CSVerificationCircuit<<E as Pairing>::BaseField>: ConstraintSynthesizer<<P as Pairing>::ScalarField>
// {
//   // Performs the circuit-specific setup required by Groth16 for the sumcheck
//   // circuit. This is done by filling the struct with dummy elements, ensuring
//   // the sizes are correct so the setup matches the circuit that will be proved.
...
//       .map(|_i| {
//         UniPoly::<E::ScalarField>::from_evals(&[
//           E::BaseField::rand(&mut rng),
//           E::BaseField::rand(&mut rng),
//           E::BaseField::rand(&mut rng),
//           E::BaseField::rand(&mut rng),
//         ])
//       })
//       .collect::<Vec<UniPoly<E::BaseField>>>();

//     let uni_polys_round2 = (0..num_vars.log_2() + 1)
//       .map(|_i| {
//         UniPoly::<E::BaseField>::from_evals(&[
//           E::BaseField::rand(&mut rng),
//           E::BaseField::rand(&mut rng),
//           E::BaseField::rand(&mut rng),
//         ])
...
//       num_cons: num_cons,
//       input: (0..num_inputs)
//         .map(|_i| E::BaseField::rand(&mut rng))
//         .collect_vec(),
//       input_as_sparse_poly: SparsePolynomial::new(
//         num_vars.log_2(),
//         (0..num_inputs + 1)
//           .map(|i| SparsePolyEntry::new(i, E::BaseField::rand(&mut rng)))
//           .collect::<Vec<SparsePolyEntry<E::BaseField>>>(),
//       ),
//       evals: (
//         E::BaseField::zero(),
//         E::BaseField::zero(),
...
//       ),
//       params: poseidon,
//       prev_challenge: E::BaseField::zero(),
//       claims_phase2: (
//         E::BaseField::zero(),
//         E::BaseField::zero(),
//         E::BaseField::zero(),
//         E::BaseField::zero(),
//       ),
//       eval_vars_at_ry: E::BaseField::zero(),
...
//         .map(|_i| E::BaseField::rand(&mut rng))
//         .collect_vec(),
//       claimed_transcript_sat_state: E::BaseField::zero(),
//     };
//     let (pk, vk) = Groth16::<P>::setup(circuit.clone(), &mut rng).unwrap();
//     CircuitGens { pk, vk, _e: PhantomData}
//   }
// }
...
//   fn prove_phase_one(
//     num_rounds: usize,
//     evals_tau: &mut DensePolynomial<E::BaseField>,
//     evals_Az: &mut DensePolynomial<E::BaseField>,
//     evals_Bz: &mut DensePolynomial<E::BaseField>,
//     evals_Cz: &mut DensePolynomial<E::BaseField>,
//     transcript: &mut PoseidonTranscript<E::BaseField>,
//   ) -> (
//     SumcheckInstanceProof<E::BaseField>,
//     Vec<E::BaseField>,
//     Vec<E::BaseField>,
//   ) {
//     let comb_func =
//       |poly_tau_comp: &E::BaseField,
//        poly_A_comp: &E::BaseField,
//        poly_B_comp: &E::BaseField,
//        poly_C_comp: &E::BaseField|
//        -> E::BaseField { (*poly_tau_comp) * ((*poly_A_comp) * poly_B_comp - poly_C_comp) };

//     let (sc_proof_phase_one, r, claims) = SumcheckInstanceProof::prove_cubic_with_additive_term(
//       &E::BaseField::zero(), // claim is zero
//       num_rounds,
//       evals_tau,
//       evals_Az,
//       evals_Bz,
...
//     num_rounds: usize,
//     claim: &E::BaseField,
//     evals_z: &mut DensePolynomial<E::BaseField>,
//     evals_ABC: &mut DensePolynomial<E::BaseField>,
//     transcript: &mut PoseidonTranscript<E::BaseField>,
//   ) -> (
//     SumcheckInstanceProof<E::BaseField>,
//     Vec<E::BaseField>,
//     Vec<E::BaseField>,
//   ) {
//     let comb_func = |poly_A_comp: &E::BaseField,
//                      poly_B_comp: &E::BaseField|
//      -> E::BaseField { (*poly_A_comp) * poly_B_comp };
...
// //   // vars.
//   pub fn prove(
//     inst: &R1CSInstance<E::BaseField>,
//     vars: Vec<E::BaseField>,
//     input: &[E::BaseField],
//     gens: &R1CSGens<E,P>,
//     transcript: &mut PoseidonTranscript<E::BaseField>,
//   ) -> (Self, Vec<E::BaseField>, Vec<E::BaseField>) {
//     let timer_prove = Timer::new("R1CSProof::prove");
//     // we currently require the number of |inputs| + 1 to be at most number of vars
//     assert!(input.len() < vars.len());
...
//       let num_vars = vars.len();
//       let mut z = vars;
//       z.extend(&vec![E::BaseField::one()]); // add constant term in z
//       z.extend(input);
//       z.extend(&vec![E::BaseField::zero(); num_vars - num_inputs - 1]); // we will pad with zeros
//       z
//     };
...
//     let timer_sc_proof_phase2 = Timer::new("prove_sc_phase_two");
//     // combine the three claims into a single claim
//     let r_A: E::BaseField = transcript.challenge_scalar(b"");
//     let r_B: E::BaseField = transcript.challenge_scalar(b"");
//     let r_C: E::BaseField = transcript.challenge_scalar(b"");
//     let claim_phase2 = r_A * Az_claim + r_B * Bz_claim + r_C * Cz_claim;
...
// //       // verifies the proof of opening against the result of evaluating the
// //       // witness polynomial at point ry
// //       println!("gens.gens_pc.vk {}",gens.gens_pc.vk.g);
// //       // println!("COMM {:?}",self.comm);
// //       // println!("RY{:?}",&ry[1..]);
// //       // println!("EVAL1 {:?}",self.eval_vars_at_ry);
// //       // println!("PROOF2 {:?}",self.proof_eval_vars_at_ry);
// //       // println!("MIPP {:?}",self.mipp_proof);
// //       // println!("T {:?}",self.t);
...
//   use crate::parameters::{PoseidonConfiguration, get_bls12377_fq_params};
//   // #[test]
//   // fn check_r1cs_proof_ark_blst() {
//   //   let params = ark_blst::Scalar::poseidon_params();
//   //   check_r1cs_proof::<ark_blst::Bls12>(params);
...
//     let num_inputs = 3;
//     let (inst, vars, input) =
//       R1CSInstance::<ark_bw6_761::Fr>::produce_synthetic_r1cs(num_cons, num_vars, num_inputs);

//     let params = crate::parameters::get_bw6_fr_params();

//     // let gens = R1CSGens::<ark_bw6_761::BW6_761>::setup(b"test-m", num_cons, num_vars, num_inputs, params.clone());

//     // // let mut random_tape = RandomTape::new(b"proof");

//     // let mut prover_transcript = PoseidonTranscript::new(&params.clone());
//     // let c = prover_transcript.challenge_scalar::<ark_bw6_761::Fr>(b"");
...
//       // let circuit =
//       // TestudoCommVerifier::<P, IV> {
//       //   vk: gens.gens_pc.vk,
//       //   U: verifer_proof.comm,
//       //   point: ry[1..].to_vec(),
//       //   v: verifer_proof.eval_vars_at_ry,
//       //   pst_proof: verifer_proof.proof_eval_vars_at_ry,
//       //   mipp_proof: verifer_proof.mipp_proof,
//       //   T: verifer_proof.t,
//       //   _iv: PhantomData,
//       // };
//   }
